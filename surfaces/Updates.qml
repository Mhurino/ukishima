...
/**
 * 更 UPDATES sub-surface: a terminal-free face for the Pill update engine. It
 * never touches git itself; it shells out to the python engine at
 * ~/.config/hypr/scripts/pill-update.py, which prints one JSON object, and
...
SettingsSurface {
    ...
    readonly property string engine: Config.hyprPath("scripts", "pill-update.py")
    ...
    FileView {
        id: manifestFile
        path: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/pill/update.json"
        watchChanges: true
        printErrors: false
    ...
    Process {
        ...
        command: ["sh", "-c",
            "d=\"${XDG_STATE_HOME:-$HOME/.local/state}/pill\"; mkdir -p \"$d\"; printf '%s' \"$1\" > \"$d/updated\"",
            "sh", body]
    }
