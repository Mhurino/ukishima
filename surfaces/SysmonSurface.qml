...
PillSurface {
    ...
    Process {
        ...
        command: ["sh", "-c",
            "out=$(curl -s -m 6 -o /dev/null -w '%{time_total}' "
            + "https://speed.cloudflare.com/meta 2>/dev/null); "
            + "rc=$?; "
    ...
    Process {
        ...
        command: ["sh", "-c",
            "bytes=0; t0=$(date +%s%N); rc=0; "
            + "while [ $(( ($(date +%s%N) - t0) / 1000000000 )) -lt 5 ]; do "
            + "b=$(head -c 25000000 /dev/zero | tr '\\0' 'x' "
            + "| curl -s -m 6 -o /dev/null "
            + "-w '%{size_upload}' -X POST "
            + "--data-binary @- "
...
Row {
    ...
    Text {
        ...
        Rectangle {
            ...
            color: root.speedRunning
                ? Qt.alpha(Theme.vermLit, 0.12)
                : (root.speedError.length > 0
                    ? Qt.alpha(Theme.verm, 0.12)
                    : Theme.frameBg)

            border.width: 1

            border.color: root.speedRunning
                ? Qt.alpha(Theme.vermLit, 0.30)
                : (root.speedError.length > 0
                    ? Qt.alpha(Theme.verm, 0.30)
                    : Theme.frameBorder)
            ...
            Column {
                ...
                Row {
                    ...
                    Column {
                        ...
                        Text {
                            text: root.speedDown > 0
                                ? root.fmtSpeed(root.speedDown)
                                : "---"
                    ...
                    Column {
                        ...
                        Text {
                            text: root.speedPing > 0
                                ? root.speedPing + " ms"
                                : "---"
                            color: root.speedPhase === "ping"
                                ? Theme.vermLit
                                : Theme.cream
                            font.family: Theme.font
                            font.pixelSize: 13 * root.s
                    ...
                    Rectangle {
                        ...
                        color: root.speedRunning
                            ? Qt.alpha(Theme.vermLit, 0.15)
                            : (root.speedError.length > 0
                                ? Qt.alpha(Theme.verm, 0.15)
                                : Qt.alpha(Theme.cream, 0.06))
