 ...
 * `$XDG_CACHE_HOME/pill/rec-thumbs`, skipping clips already cached) and only
...
Singleton {
    ...
    readonly property string thumbDir: (Quickshell.env("XDG_CACHE_HOME") || (home + "/.cache")) + "/pill/rec-thumbs/"
    ...
    Process {
        ...
        onExited: function(exitCode) {
            ...
            if (exitCode !== 0) {
                ...
                failProc.command = ["notify-send", "-a", "Pill", "-u", "critical",
    ...
    Process {
        id: savedProc
        command: ["notify-send", "-a", "Pill", "Recording saved",
